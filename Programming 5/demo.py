...
def parse_arg():
    ...
    parser.add_argument('--image_path', type=str, default='test', help='the path to your image')
    parser.add_argument('--checkpoint', type=str, default='output/OWN/crnn/2021-06-10-18-10/checkpoints/checkpoint_18_acc_0.7294.pth',
...
if __name__ == '__main__':
    ...
    info = {}
    started = time.time()
    file_list = os.listdir("./data/" + args.image_path)
    for filename in file_list:
        print(f"./data/{args.image_path}/{filename}", end=" ------> ")
        img = cv2.imread(f"./data/{args.image_path}/{filename}")
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        converter = utils.strLabelConverter(config.DATASET.ALPHABETS)

        output = recognition(config, img, model, converter, device)
        info[filename] = output
        print(output)
    json_str = json.dumps(info, ensure_ascii=False)
    with open(f"./{args.image_path}1.json", 'w', encoding='utf-8') as f:
        f.write(json_str)
    finished = time.time()
